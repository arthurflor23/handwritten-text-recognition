...
class SynthesisModel(SynthesisBaseModel):
    ...
    def train_step(self, input_data):
        ...
        with tf.GradientTape() as e_tape, \
            ...
            # generator loss
            g_loss = (kl_loss * 1e-4) + info_loss + l1_loss + disc_loss + ctc_loss + wid_loss + (ctx_loss * 2.0)
...
class BackboneModel(tf.keras.Model):
    ...
    def build_model(self):
        ...
        conv = tf.keras.layers.Conv2D(self.blocks[0], kernel_size=5, strides=1, padding='same')(image_inputs)
